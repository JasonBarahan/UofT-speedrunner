...
def _visualize_complete_path(m: folium.Map, edges: list[list[ent.Edge]], start: ent.Building, end: ent.Building,
                             intermediate_stops: list[ent.Building] | None = None) -> None:
    """
...
def visualize_djikstra_with_stopovers(start: str, end: str, stopovers: list[str]) -> None:
    """
    ...
    - all elements of intermediate_stops are valid building ids
    """
    datum = load_all_data.load_data('data/building_data.csv', 'data/intersections_data.csv')
    dji = load_all_data.DijkstraGrid(datum.intersections, datum.buildings)
    m = generate_map("OpenStreetMap")
    ...
    all_paths = [main_path_edges]  # list[list[Edge]]
    for stop in stopovers:
        stop_building = building_data[stop]
        stop_intersection = stop_building.closest_intersection
        intersection_id_with_shortest_distance = None
        distance = math.inf
        for edge in main_path_edges:
            ...
            for intersection in edge.endpoints:
                distance_between_intersection_and_stop = ent.get_distance(intersection.coordinates, stop_building.coordinates)
                if distance_between_intersection_and_stop < distance:
                    distance = distance_between_intersection_and_stop
                    intersection_id_with_shortest_distance = intersection.identifier
        optimal_path = dji.find_shortest_path(intersection_id_with_shortest_distance, stop_intersection.identifier)
        all_paths.append(optimal_path)

    _visualize_complete_path(m, all_paths, start_building, end_building)
