...
def load_data(building_file: str, intersection_file: str) -> AbstractGrid:
    ...
    with open(intersection_file) as intersection_file:
        ...
        for i in range(1, len(intersections)):
            this_intersection = Intersection(
                int(intersections[i][0]), {intersections[i][1], intersections[i][2]},
                (float(intersections[i][3]), float(intersections[i][4])))
            intersections_dict[int(intersections[i][0])] = this_intersection

        # TODO: need to change this with a concrete class by addind a check with if statements
    ...
    for building in my_grid.buildings:
        ...
        print("This is my_grid.intersections baby! {}".format(my_grid.intersections))
        print("This is the closest intersection for {} right here: {}".format(building, my_grid.buildings[building].closest_intersection))
        my_grid.buildings[building].closest_intersection = my_grid.intersections[intersection_id]
    ...
    for intersection in my_grid.intersections:
        ...
        my_grid.intersections[intersection].close_buildings = close_buildings_set
